...
public class AgendamentoDeConsultas {
    ...
    public void agendarConsulta(DadosAgendarConsulta dados) {
        if (!petRepository.existsById(dados.idPet())) {
            throw new ValidacaoException("o id do pet informado não existe no banco de dados");
        }
        if(dados.idVeterinario() != null && !veterinarioRepository.existsById(dados.idVeterinario())) {
            throw new ValidacaoException("o id do veterinário informado não existe no banco de dados");
        }
    ...
    private Veterinario escolherVeterinario(DadosAgendarConsulta dados) {
        ...
        if(dados.especialidade() == null) {
            throw new ValidacaoException("Especialidade é obnrigatória quando não tiver id de veterinário");
        }
